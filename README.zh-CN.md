...
智能模糊是录制过程中你主动进入的模式，而不是一直生效的过滤器。点击 **智能模糊** 后捕获会暂停，Mimik 会检测并遮盖页面上的敏感数据（邮箱、电话号码、SSN、信用卡、IP 地址、MAC 地址），点击**完成**之后，该页面的截图会保持遮盖。每个类别都可以独立开关。
...
在导出面板中开启后，视频的每个步骤都会被朗读出来，使用你自己的 OpenAI 或 ElevenLabs 密钥。如果你
已经为 AI 描述配置了 OpenAI 密钥，Mimik 会直接复用，无需再注册其他服务。有配音的步骤会一直停留到
语音结束，因此不会被截断；音频片段会缓存在本地，重新导出同一份指南不会产生额外费用。
...
- **视频**：带讲解的 walkthrough，mp4/H.264，光标会移动到每个目标元素 —— 可选启用 ElevenLabs 配音朗读每个步骤，
  同时让视频满足第 508 条无障碍要求
- **PDF**：适合打印，A4 纵向，自动分页
